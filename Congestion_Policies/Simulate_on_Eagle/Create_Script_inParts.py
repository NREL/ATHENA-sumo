...
time_in_hours = 8   # Run time of job on eagle, float
...
sim_seconds = math.floor(day_secs/num_scripts)  # seconds to simulat per part of simulation
...
print("Max simulation time:", max_hours, "hours\nNumber of scripts to create:", num_scripts, "\nSumo simulation time per script:", sim_seconds, "sec\nEagle Allocated time per script: ", hours_per_part)
...
for i in range(1,num_scripts+1):
    ...
    if begin_year == end_year: script_file = str(e)+ "_run_sim_"+ str(begin_year) + '.slurm'
    else: script_file = str(e)+ "_run_sim_"+ str(begin_year) + '_' + str(end_year)+'.slurm'
